...
Visitors click their arch directly on the 3D site model, choose a room, and receive a highlighted route through the building. On phones, the ribbon, map, current task, and copyright share one screen without overlapping. “Guide Me” presents one concise instruction and one matching landmark photo at a time beside a locked 3D view of the instruction’s actual floor.
...
- Mobile step sequencing that keeps the current task in place without moving the document
...
scripts/validate-layout.mjs      Viewport lifecycle and camera-fit checks
...
- The first visible map is rendered after the current task panel has been laid out; no initial scroll is required.
- The map occupies its own grid row, and camera framing uses that row's dimensions. The county ribbon, view controls, task panel, and copyright cannot cover it.
- Browser-toolbar changes, page restoration, and orientation changes update the viewport and camera. Pinch zoom leaves the layout size intact.
- Landscape phones show the map beside the current task. Guidance instructions wrap, and photos retain their original proportions without cropping.
- This update preserves the modeled rooms, elevators, stairs, route graph, direction wording, and Read Aloud behavior.

`npm run build` includes the existing wayfinding checks plus viewport lifecycle, camera projection, first-frame sequence, and JavaScript syntax checks. These automated checks are not a browser or on-device visual sign-off. Before the presentation, open the deployed version in iPhone Safari with the address bar expanded, reload without scrolling, choose each arch, run guidance, and rotate the phone. Verify the complete building and usable controls on the first frame, then check Read Aloud on the presentation device.
